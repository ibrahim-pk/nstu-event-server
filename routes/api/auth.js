...
router.route('/venues/:id').get((req, res) => {
 ...
 router.route('/confirmation').post((req, res) => {
  //console.log(req.body)
     ...
     const updateData= Events.findOneAndUpdate({_id: req.body.event_id}, {$addToSet: {attendees: req.body.user_id}})
  ...
  .then(user=>{
    ...
    user.save().then( console.log('saved'));
    res.status(200).send({msg:`Cancel your event+${req.body.activityname}`})
